...
export default function DashboardPage() {
    ...
    const [plan, setPlan] = useState<typeof MOCK_PLAN>(MOCK_PLAN);
        ...
        <div className="min-h-screen bg-linear-to-b from-black to-slate-900 text-slate-200 pb-20 pt-16">
            ...
            <main className="max-w-5xl mx-auto px-4 py-8">
                ...
                <section className="mb-8">
                    <h2 className="text-3xl font-bold text-white mb-2">Ready to crush it, Rahul? 🚀</h2>
                    <p className="text-slate-400 max-w-2xl">{plan.summary}</p>

                    {/* Stats Row */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                        {[
                            { label: "Focus", val: "Muscle Gain", icon: Flame, color: "text-orange-500" },
                            { label: "Split", val: "Push/Pull/Legs", icon: Calendar, color: "text-blue-500" },
                            { label: "Calories", val: "2,400 / day", icon: Utensils, color: "text-green-500" },
                            { label: "Duration", val: "60 mins", icon: Clock, color: "text-purple-500" },
                        ].map((stat, i) => (
                            <div key={i} className="p-4 rounded-xl bg-slate-900 border border-slate-800 flex flex-col gap-1">
                                <div className="flex items-center gap-2 text-sm text-slate-400">
                                    <stat.icon size={16} className={stat.color} /> {stat.label}
                                </div>
                                <div className="font-bold text-white text-lg">{stat.val}</div>
                            </div>
                        ))}
                    </div>
                </section>
                ...
                <div className="flex gap-2 mb-8 p-1 bg-slate-900/50 rounded-xl border border-slate-800 w-full md:w-fit">
                    {[
                        ...
                        { id: "analysis", label: "AI Coach Tips", icon: Brain },
                    ].map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id as any)}
                            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === tab.id
                        ...
                        >
                            ...
                            <div className="grid gap-4">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <button className="text-xs flex items-center gap-1 text-blue-400 hover:underline">
                                        <Volume2 size={14} /> Read Routine
                                    </button>
                                </div>

                                {plan.workout.schedule[selectedDay].exercises.map((ex, i) => (
                                    <div key={i} className="group relative bg-slate-900 border border-slate-800 rounded-2xl p-5 hover:border-slate-600 transition-all">
                                        <div className="flex justify-between items-start">
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <h4 className="font-bold text-lg text-white">{ex.name}</h4>
                                                    <span className="text-xs px-2 py-0.5 bg-slate-800 text-slate-400 rounded-full border border-slate-700">{ex.focus}</span>
                                                </div>
                                                ...
                                                <div className="flex gap-4 text-sm">
                                                    <div className="px-3 py-1.5 bg-slate-950 rounded-lg border border-slate-800 text-slate-300">
                                                        <span className="text-slate-500 text-xs block">SETS</span> {ex.sets}
                                                    </div>
                                                    <div className="px-3 py-1.5 bg-slate-950 rounded-lg border border-slate-800 text-slate-300">
                                                        <span className="text-slate-500 text-xs block">REPS</span> {ex.reps}
                                                    </div>
                                                    <div className="px-3 py-1.5 bg-slate-950 rounded-lg border border-slate-800 text-slate-300">
                                                        <span className="text-slate-500 text-xs block">REST</span> {ex.rest}
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    {activeTab === "diet" && (
                        ...
                        >
                            <div className="flex justify-between items-center">
                                ...
                                <button className="text-xs flex items-center gap-1 text-green-400 hover:underline">
                                    <Volume2 size={14} /> Listen to Plan
                                </button>
                            </div>

                            {plan.diet.map((meal, i) => (
                                <div key={i} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col md:flex-row gap-6 items-start md:items-center">
